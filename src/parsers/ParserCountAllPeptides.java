 ...
 */
public class ParserCountAllPeptides {
    static {
        try {
            System.load("lib/native/msparserj.dll");
...
//        printLHM2("tmp20/uniquePeptidesToProteins.txt", uniquePeptides2proteins);
        LinkedHashMap<String, LinkedHashSet<String>> proteins2peptides = makeTableProteins2peptides(peptides2proteins);
    ...
    private static boolean isBactProtein(ms_protein protein) {
        return protein.getAccession().startsWith("GL");
    }
    ...
    private static LinkedHashMap<String, LinkedHashSet<String>> makeTableProteins2peptides(LinkedHashMap<String, LinkedHashSet<String>> lhm) {
        LinkedHashMap<String, LinkedHashSet<String>> result = new LinkedHashMap<>();
        for (Map.Entry<String, LinkedHashSet<String>> entry : lhm.entrySet()) {
            if (result.containsKey(entry.getKey())) {
                LinkedHashSet<String> l1 = result.get(entry.getKey());
                l1.add(entry.getKey());
                result.put(entry.getKey(), l1);
            } else {
